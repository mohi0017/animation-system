...
# Model URL - Set internally (GitHub Release)
MODEL_URL = "https://github.com/mohi0017/animation-system/releases/download/v1.0.0/epoch_014.pth"
MODEL_FILENAME = "epoch_014.pth"

# Download model function (cached)
@st.cache_resource
...
# Auto-download model
checkpoint_path = download_model(MODEL_URL, MODEL_FILENAME)
...
with st.sidebar:
    ...
    if checkpoint_path and os.path.exists(checkpoint_path):
        ...
        st.caption(f"Source: GitHub Release")
    else:
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 Process Image", type="primary"):
            if checkpoint_path is None or not os.path.exists(checkpoint_path):
                with st.spinner("Downloading model from GitHub Release... This may take 1-2 minutes."):
                    checkpoint_path = download_model(MODEL_URL, MODEL_FILENAME)
                if checkpoint_path is None:
                    st.error("⚠️ Failed to download model. Please try again.")
                    st.stop()
